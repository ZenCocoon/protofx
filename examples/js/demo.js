...
function fxControl(fx, includePlayer, includeOptions) {
  var player = ["<button id='play'>    Play    </button>",
                ...
                "<input type='checkbox' id='reverse' autocomplete='off'/> Reverse ",
                "<br/>",
                "<br/>"].join('\n');
  var options = ["Duration in ms <input type='text' id='duration' value='1000'/>",
                "Transition",
                "<select id='transition'>",
                "</select>",
                "<button id='set_options'>Set</button>",
                "<br/>",
                "<br/>"].join('\n');
  var control = $('fx_controls');
  if (!control) {
    control = new Element('div');
    control.setAttribute('id', 'fx_controls');
    $(document.body).insert({top: control});
  }
    
  if (includePlayer) {
    control.innerHTML += player;
  }
  if (includeOptions) {
    control.innerHTML += options;
    $('duration').value = fx.options.duration;
  }
  ...
  if (includePlayer) {
    ...
    $('reverse').observe('click', function() {
      fx.reverse();
    });
    ...
    $('rewind').observe('click', function() {
      fx.rewind();
    });
  }
  
  if (includeOptions) {
    $('set_options').observe('click', function() {
      fx.options.duration = $('duration').value;
      var selected = $('transition').options[$('transition').selectedIndex].value;
      fx.options.transition = FX.Transition[selected];
    });
  
    var html = ""
    for (i in FX.Transition) {
      html += '<option value="' + i + '"'
      if (FX.Transition[i] == fx.options.transition)
        html += ' selected=selected '
      html += '>' + i + '</option>'
    }
    $('transition').update(html);
  }
}